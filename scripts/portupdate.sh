...
[ "$2" ] && {
	# update version
	sed -i "/^version=/s/=.*/=$2/" $PORTSDIR/$1/hpkgbuild

	# change release to 1
	sed -i "/^release=/s/=.*/=1/" $PORTSDIR/$1/hpkgbuild
}
...
if [ "$NOPROMPT" = 1 ]; then
	...
	while true; do
		...
		while true; do
			...
			echo
			case $input in
				E|e) $EDITOR $PORTSDIR/$1/hpkgbuild
					 break 1;;
				A|a) exit 1;;
				C|c) break 2;;
			esac
