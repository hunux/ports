#!/bin/bash
#
# script to operate Hunux GNU/Linux through chroot environment
#
...
umount_cache_and_portsrepo() {
	...
	unmount $ROOTFS/var/cache/hunpkg/packages
	unmount $ROOTFS/var/cache/hunpkg/sources
...
mount_cache_and_portsrepo() {
	...
	bindmount $SRCDIR $ROOTFS/var/cache/hunpkg/sources
	bindmount $PKGDIR $ROOTFS/var/cache/hunpkg/packages
...
fetch_rootfs() {
	url="https://github.com/hunux/ports/releases/download/v$RELEASE/hunux-rootfs-$RELEASE-$ARCH.tar.xz"
...
set_release_info() {
	echo "$RELEASE" > "$ROOTFS"/etc/hunux-release
	sed -i "s/PRETTY_NAME=.*/PRETTY_NAME=\"Hunux GNU/Linux $RELEASE\"/" "$ROOTFS"/etc/os-release
...
compress_rootfs() {
	...
	tar --exclude="var/cache/hunpkg/packages/*" \
		--exclude="var/cache/hunpkg/sources/*" \
		--exclude="var/cache/hunpkg/work/*" \
		--exclude="*.hpkgnew" \
		--exclude="tmp/*" \
		--exclude="root/*" \
...
tmp_hunpkgconf() {
	mv "$ROOTFS"/etc/hunpkg.repo "$ROOTFS"/etc/hunpkg.repo.hpkgnew
	for i in $REPO; do
		echo "/usr/ports/$i" >> "$ROOTFS"/etc/hunpkg.repo
	done
	cp "$ROOTFS"/etc/hunpkg.conf "$ROOTFS"/etc/hunpkg.conf.hpkgnew
	sed "s/MAKEFLAGS=.*/MAKEFLAGS=\"-j$JOBS\"/" -i "$ROOTFS"/etc/hunpkg.conf
}

main_hunpkgconf() {
	chrootrun hun install -r -y --no-backup hunpkg
	#if [ -f $ROOTFS/etc/hunpkg.repo.hpkgnew ]; then
	#	mv $ROOTFS/etc/hunpkg.repo.hpkgnew $ROOTFS/etc/hunpkg.repo
	#fi
	#if [ -f $ROOTFS/etc/hunpkg.conf.hpkgnew ]; then
	#	mv $ROOTFS/etc/hunpkg.conf.hpkgnew $ROOTFS/etc/hunpkg.conf
	#fi
}
...
make_iso() {
	...
	main_hunpkgconf
	copy_ports
	#chrootrun hun install -y hunpkg
	#sed "s/MAKEFLAGS=.*/MAKEFLAGS=\"-j\$(nproc\)\"/" -i "$ROOTFS"/etc/hunpkg.conf

	# make sfs
	msg "Squashing root filesystem: $ISODIR/rootfs/filesystem.sfs ..."
	mksquashfs "$ROOTFS" "$ISODIR/rootfs/filesystem.sfs" \
			-b 1048576 -comp zstd \
			-e "$ROOTFS"/var/cache/hunpkg/sources/* \
			-e "$ROOTFS"/var/cache/hunpkg/packages/* \
			-e "$ROOTFS"/var/cache/hunpkg/work/* \
			-e "$ROOTFS"/root/* \
			-e "$ROOTFS"/tmp/* \
			-e "*.hpkgnew" 2>/dev/null || die "Failed create sfs root filesystem"

	cp "$ROOTFS/boot/vmlinuz-hunux" "$ISODIR/boot/vmlinuz" || die "Failed copying kernel"

	sed "s/@ISOLABEL@/$ISOLABEL/g" "$FILESDIR/hunuxiso.hook" > "$ROOTFS/etc/mkinitramfs.d/hunuxiso.hook" || die "Failed preparing hunuxiso.hook"
	kernver=$(file $ROOTFS/boot/vmlinuz-hunux | cut -d ' ' -f9)
	chrootrun mkinitramfs -k $kernver -a hunuxiso || die "Failed create initramfs"
	cp "$ROOTFS/boot/initrd-hunux.img" "$ISODIR/boot/initrd" || die "Failed copying initrd"
	...
	mkdosfs -n HUNUX-UEFI "$ISODIR/boot/efiboot.img" || die "Failed create mkdosfs image"
...
main() {
	...
	[ "$SYSUP" ] && {
		msg "Upgrading hunpkg..."
		chrootrun hun upgrade hunpkg -y --no-backup || die
		tmp_hunpkgconf
		msg "Full upgrading..."
		chrootrun hun sysup -y --no-backup || die
	}
	...
	[ "$CCACHE" ] && {
		chrootrun hun install -y ccache || die
	}

	[ "$PKG" ] && {
		chrootrun hun install -y $(echo $PKG | tr ',' ' ') || die
	}

	[ "$REBUILDPKG" ] && {
		chrootrun revdep -r -y || die
		chrootrun hun install -fr $REBUILDPKG || die
	}
	...
	[ "$ISO" ] && {
		chrootrun hun install -y $(echo $ISO_PKG | tr ',' ' ') || die
		[ "$REVDEP" ] && {
...
TARBALLIMG="$PORTSDIR/hunux-rootfs-$RELEASE-$ARCH.tar.xz"
SRCDIR="${SRCDIR:-/var/cache/hunpkg/sources}"
PKGDIR="${PKGDIR:-/var/cache/hunpkg/packages}"
...
REPOFILE="$FILESDIR/hunpkg.repo"

# iso
ISODIR="${ISODIR:-/tmp/hunuxiso}"
ISOLABEL="HUNUXLIVE_$(date +"%Y%m%d")"
ISO_PKG="linux-lts,dialog,squashfs-tools,grub-efi,btrfs-progs,reiserfsprogs,xfsprogs,syslinux"
OUTPUTISO="${OUTPUTISO:-$PORTSDIR/hunux-$RELEASE-$ARCH.iso}"
