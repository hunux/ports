...
getdeps() {
	# getdeps <portpath>
	grep "^# depends[[:blank:]]*:" $portpath/hpkgbuild \
...
verifyvar() {
	for i in backup noextract source; do
		grep -q ${i}=\"\" $portpath/hpkgbuild && msg "please remove this empty variable: ${i}=\"\""
	done
}
...
verifythisisnotcrux() {
	for i in PKGMK prt prt-get; do
		grep -q "$i" $portpath/hpkgbuild && msg "please remove CRUX stuff, this is not CRUX"
	done
}

verifythisisnotarch() {
	for i in pkgver pkgrel pkgdesc pkgdir srcdir; do
		grep -q "$i" $portpath/hpkgbuild && msg "please remove ARCH stuff, this is not ARCH"
	done
}
...
verifynotusedir() {
	...
	for i in \
		...
		usr/etc/ \
		usr/libexec/; do
		grep -q ${i}$ $portpath/.pkgfiles && msg "this directory not use in hunux: ${i}"
	done
}

verifydeps() {
	if [ "$(grep "^# depends[[:blank:]]*:" $portpath/hpkgbuild)" ]; then
		deps=$(getdeps)
		[ "$deps" ] || {
