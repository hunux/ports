...
port_update() {
	...
	# update version
	sed -i "/^version=/s/=.*/=$vers/" hpkgbuild
	
	# change release to 1
	sed -i "/^release=/s/=.*/=1/" hpkgbuild
	...
	sudo pkgbuild -u -v  && sudo hun trigger ${portpath#*/}
...
port_checkdep() {
	for r in ${PORTREPO[@]}; do
		tmprepo="$tmprepo $r"
		for p in $r/*/hpkgbuild; do
			portpath=$(dirname $p)
			for d in $(get_depends); do
				found=0
				for tp in $tmprepo; do
					if [ -f $tp/$d/hpkgbuild ]; then
						found=1
						break
					fi
				done
...
INDEX_DIR="/var/lib/hunpkg/index"
EDITOR=${EDITOR:-vim}
